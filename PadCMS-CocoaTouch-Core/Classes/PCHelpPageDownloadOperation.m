...
@implementation PCHelpPageDownloadOperation

@synthesize filePath, helpPageKey = _helpPageKey, operationTarget, progressTarget;
...
- (id)initWithResource:(NSString*)resource forKey:(NSString*)key
{
        ...
        ||[pathExtension isEqualToString:@"jpeg"])
    {
        ...
        {
            urlStr = [[PCConfig serverURLString] stringByAppendingFormat:@"/resources/768-1024%@", resource];
        }
    } 
    else 
    {
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if (operationTarget&&[operationTarget respondsToSelector:@selector(endDownloadingPCHelpPageDownloadOperationOperation:)]) 
    {
        [operationTarget performSelectorOnMainThread:@selector(endDownloadingPCHelpPageDownloadOperationOperation:) withObject:self waitUntilDone:YES];
    }
}

- (void)finishprogressDownloading
{
    [self.progressTarget progressOfDownloading:100.0f forHelpPageKey:self.helpPageKey];
}
    
- (void)progressOfDownloadOperation
{
    CGFloat progress = ((CGFloat) _lengthOfDownloadedData / (CGFloat)_expectedContentLength);
	[self.progressTarget progressOfDownloading:progress forHelpPageKey:self.helpPageKey];
}
